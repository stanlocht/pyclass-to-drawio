...
class ClassDiagramGenerator:
    ...
    def generate_diagram(
        self, module_name, file_name=None, direction="down", link_style="orthogonal"
    ):
        """
        ...
        for class_name in self.all_classes:
            nodes[class_name] = NodeObject(
                tree=tree, value=class_name, base_style="rounded rectangle"
            )
        ...
        for class_name, cls in classes.items():
            ...
            try:
                ...
                for other_class_name in classes.keys():
                    ...
                    if (
                        f"self.{other_class_name.lower()}" in source
                        or f"self._{other_class_name.lower()}" in source
                        or f" {other_class_name}(" in source
                    ):
...
def generate_class_diagram(
    module_name, output_dir=".", file_name=None, direction="down", link_style="orthogonal"
):
    """
    ...
    return generator.generate_diagram(
        module_name=module_name, file_name=file_name, direction=direction, link_style=link_style
    )
