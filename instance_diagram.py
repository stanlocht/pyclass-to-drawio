...
def create_instance_diagram(output_dir=None, file_name="pet_instances_diagram.drawio"):
    ...
    nodes["whiskers"] = NodeObject(tree=tree, value="Whiskers (Cat)", base_style="rounded rectangle")
    ...
    # Vet node
    nodes["dr_smith"] = NodeObject(tree=tree, value="Dr. Smith (Vet)", base_style="rounded rectangle")
    nodes["dr_smith"].apply_style_string("fillColor=#fff2cc;strokeColor=#d6b656;")  # Yellow
    
    # Pet shop node
    nodes["pet_palace"] = NodeObject(tree=tree, value="Pet Palace (Shop)", base_style="rounded rectangle")
